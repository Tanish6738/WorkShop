...
export default function AdminCollections(){
  ...
  const load = async (page=1)=>{ setLoading(true); setError(null); try { const res = await listAllCollections({ page, q, visibility }); if(res?.data) setData(res.data); } catch(e){ setError(e?.error?.message||'Failed'); } finally { setLoading(false);} };
  useEffect(()=>{ load(1); },[]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-3 items-end">
        <div className="flex flex-col gap-1">
          ...
          <input value={q} onChange={e=> setQ(e.target.value)} onKeyDown={e=> { if(e.key==='Enter') load(1); }} placeholder="Name or description" className="px-3 py-2 rounded-md bg-[var(--pv-surface-alt)] border border-[var(--pv-border)] text-xs focus:outline-none focus:ring-2 focus:ring-[var(--pv-orange)]/60" />
        </div>
        <div className="flex flex-col gap-1">
        ...
        {selection.length>0 && (
          <div className="flex gap-2 items-center mt-5">
            <button onClick={()=> act(bulkDeleteCollections, selection)} className="px-3 py-2 rounded-md text-[11px] font-medium bg-red-600/80 hover:bg-red-600 text-white">Delete ({selection.length})</button>
          </div>
        )}
      </div>
            ...
            <tbody>
              {data.items.map(c=> (
                <motion.tr key={c._id} initial={{ opacity:0 }} animate={{ opacity:1 }} className="border-t border-[var(--pv-border)] hover:bg-[var(--pv-surface-hover)]/40">
                  <td className="px-3 py-2 align-top"><input type="checkbox" checked={selection.includes(c._id)} onChange={()=> toggleSel(c._id)} /></td>
                  ...
                  <td className="px-3 py-2 align-top">
                    <button onClick={()=> act(deleteCollectionAdmin, c._id)} className="px-2 py-1 rounded bg-red-600/80 hover:bg-red-600 text-white text-[10px]">Delete</button>
                  </td>
                </motion.tr>
              ))}
            </tbody>
